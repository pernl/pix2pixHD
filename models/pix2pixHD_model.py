...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionFeedback = torch.nn.NLLLoss2d(ignore_index=255)
            if not opt.no_vgg_loss:
                self.criterionVGG = networks.VGGLoss(self.gpu_ids)

            # Names so we can breakout loss
            self.loss_names = ['G_GAN', 'G_GAN_Feat', 'G_VGG', 'D_real', 'D_fake', 'feedback']
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        model.cuda()
        model.eval()
        input_label2 = wrap_cuda(torch.squeeze(label.type(torch.LongTensor), 0))
        segm_fake_image = model.forward(fake_image)[0]
        loss_feedback = self.criterionFeedback(segm_fake_image, input_label2)
        ...
        return [ [ loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_D_real, loss_D_fake, loss_feedback ], None if not infer else fake_image ]
